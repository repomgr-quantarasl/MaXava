...
class Android extends MY_Controller
{
	...
	{
		$output = array();
		$tablelist = $this->Api_v2_model->read_all('*', 'rest_table', '', '', 'tableid', 'ASC');
		if (!empty($tablelist)) {
			$i = 0;
			foreach ($tablelist as $table) {
				$output['tableinfo'][$i]['tableid']                    = $table->tableid;
				$output['tableinfo'][$i]['tablename']                  = $table->tablename;
				$output['tableinfo'][$i]['person_capicity']            = $table->person_capicity;
				$output['tableinfo'][$i]['table_icon']                 = $table->table_icon ? base_url() . $table->table_icon : '';
				$output['tableinfo'][$i]['status']                     = $table->status;
				$i++;
			}
	...
	public function placeorder()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$orderinfo['table_no']  		= $this->input->post('table') ? $this->input->post('table') : 0;
			...
			if ($this->Api_v2_model->orderitem($orderid, $customerid)) {
				...
				if ($paymentsatus == 5) {
					...
					// Configure M-Pesa with payment setup
					$config = array(
						'api_key' => $paymentsetup->marchantid,
						'public_key' => $paymentsetup->password,
						'service_provider_code' => $paymentsetup->email,
						'is_live' => $paymentsetup->Islive == 1 ? true : false
					);
					
					$this->mpesa_lib->initialize($config);
					
					// Prepare M-Pesa payment data
					$output['payment_type'] = 'mpesa';
					$output['order_id'] = $orderid;
					$output['amount'] = $this->input->post('grandtotal');
					$output['phone'] = $this->input->post('phone');
					$output['customer_name'] = $this->input->post('full_name');
					$output['reference'] = 'ORD_' . $orderid;
					$output['description'] = 'Order payment for ' . $orderid;
					$output['success_url'] = base_url() . "android/mpesa_success/" . $orderid;
					$output['cancel_url'] = base_url() . "android/mpesa_cancel/" . $orderid;
					$output['callback_url'] = base_url() . "android/mpesa_callback/" . $orderid;
					
					return $this->respondWithSuccess('M-Pesa payment initiated', $output);
				} else {
					$scan = scandir('application/modules/');
					$pointsys = "";
					foreach ($scan as $file) {
...
<?php
	...
	// M-Pesa Payment Callback Methods
	public function mpesa_success($orderid = null)
	{
		if ($orderid) {
			...
			$this->Api_v2_model->update_date('customer_order', $updateData, 'order_id', $orderid);
			
			$output['status'] = 'success';
			$output['message'] = 'Paiement M-Pesa réussi';
			$output['order_id'] = $orderid;
			return $this->respondWithSuccess('Paiement M-Pesa réussi', $output);
		} else {
			return $this->respondWithError('ID de commande invalide', array());
		}
	}

	public function mpesa_cancel($orderid = null)
	{
		if ($orderid) {
			// Update order status to cancelled
			$updateData = array(
				'order_status' => 3, // Cancelled status
				'PaymentStatus' => 0
			);
			$this->Api_v2_model->update_date('customer_order', $updateData, 'order_id', $orderid);
			
			$output['status'] = 'cancelled';
			$output['message'] = 'Paiement M-Pesa annulé';
			$output['order_id'] = $orderid;
			return $this->respondWithError('Paiement M-Pesa annulé', $output);
		} else {
			return $this->respondWithError('ID de commande invalide', array());
		}
	}

	public function mpesa_callback($orderid = null)
	{
		...
		if ($orderid && !empty($data)) {
			// Log the M-Pesa transaction
			$transactionData = array(
				'order_id' => $orderid,
				'phone_number' => isset($data['phone']) ? $data['phone'] : '',
				'amount' => isset($data['amount']) ? $data['amount'] : 0,
				'transaction_reference' => isset($data['transaction_id']) ? $data['transaction_id'] : '',
				'mpesa_response_code' => isset($data['response_code']) ? $data['response_code'] : '',
				'mpesa_response_desc' => isset($data['response_desc']) ? $data['response_desc'] : '',
				'mpesa_conversation_id' => isset($data['conversation_id']) ? $data['conversation_id'] : '',
				'mpesa_transaction_id' => isset($data['transaction_id']) ? $data['transaction_id'] : '',
				'status' => isset($data['status']) ? $data['status'] : 'pending',
				'created_at' => date('Y-m-d H:i:s')
			);
			
			// Check if table exists before inserting
			if ($this->db->table_exists('mpesa_transactions')) {
				$this->db->insert('mpesa_transactions', $transactionData);
			}
			...
			$output['message'] = 'Callback M-Pesa traité';
			return $this->respondWithSuccess('Callback M-Pesa traité', $output);
		} else {
			return $this->respondWithError('Données de callback invalides', array());
		}
	}
}
