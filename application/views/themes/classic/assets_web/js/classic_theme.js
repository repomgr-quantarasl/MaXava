...
$(document).ready(function() {
	...
	// Initialize clockpicker for reservation time field
...
function getcheckbox(price, name, rate_type) {
    var subtotal = parseFloat($("#subtotal").text());
    var servicecharge = 0;

    // 1. Rate type check
    if(rate_type === 'amount') {
        servicecharge = parseFloat(price); // fixed amount
    } else if(rate_type === 'percentage') {
        servicecharge = subtotal * parseFloat(price) / 100; // percentage of subtotal
    }

    // 2. Set values in HTML
    $("#scharge").text(servicecharge.toFixed(2));
    $("#servicename").val(name);
    $("#getscharge").val(servicecharge.toFixed(2));

    // 3. Get other amounts
    var vat = parseFloat($("#vat").text());
    var discount = parseFloat($("#discount").text());
    var totalprice = parseFloat($("#subtotal").text());
    var coupondis = parseFloat($("#coupdiscount").text());

    // 4. Calculate grand total
    var grandtotal = 0;
    if(basicinfo.isvatinclusive == 1){
        grandtotal = (totalprice + servicecharge) - (discount + coupondis);
    } else {
        grandtotal = (totalprice + vat + servicecharge) - (discount + coupondis);
    }
    ...
    var dataString = "shippingcharge=" + servicecharge + '&shipname=' + name + '&csrf_test_name=' + basicinfo.csrftokeng;
...
function gotocheckout() {
    ...
    // Check if table selection is required but not selected
    var selectedShippingMethod = $('input[name="payment_method"]:checked').siblings('label').text().trim();
    if (selectedShippingMethod.indexOf('Consumir no Local') !== -1 && $("#tableid").val() === "") {
        error = 1;
    ...
    if (error == 0) {
        // Store table selection in session storage if dining in
        if (selectedShippingMethod.indexOf('Consumir no Local') !== -1) {
            sessionStorage.setItem('selected_table_id', $("#tableid").val());
        } else {
