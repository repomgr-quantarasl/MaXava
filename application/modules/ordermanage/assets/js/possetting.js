  ...
  $(window).on('load', function(){
      ...
      var myurl = basicinfo.baseurl.replace(/\/+$/, '') + "/ordermanage/order/cashregister";
  ...
  $('body').on('click', '.select_product', function(e) {
      ...
      if (hasaddons == 0 && totalvarient == 1 && customqty == 0) {
          ...
          var cartAudio = new Audio(mysound + audio[0]);
          var playPromise = cartAudio.play();
          if (playPromise !== undefined) {
              playPromise.catch(function(error) {
                  console.log("Cart sound autoplay prevented:", error);
              });
          }
...
$(document).on("keypress", '#itemqty_1', function(e){
 ...
 $("#edit").on('shown.bs.modal', function () {
  ...
  function placeorder() {
    ...
    var thirdinvoiceid = "";
    var tableid = "";
    var customer_name = $("#customer_name").val(); // not required now
    var cardtype = 4;
    var isonline = 0;
    ...
    // ❌ Customer name validation removed
    ...
    if (ctypeid == 3) {
        ...
        if (isdelivary == '') {
            errormessage = errormessage + '<span>Please Select Delivery Company.</span>';
        ...
        var table_member = $("#table_member").val();
        if (possetting.waiter == 1) {
            if (waiter == '') {
                errormessage = errormessage + '<span>Please Select Waiter.</span>';
                $("#waiter").select2('open');
        ...
        if (possetting.tableid == 1) {
            ...
            // ✅ table_person check removed (optional now)
        }
    }
    ...
    if (errormessage == '') {
        ...
        var dataString =
            ...
            '&assigncard_terminal=&assignbank=&assignlastdigit=' +
  ...
  // Global audio context enabler
  var audioContextEnabled = false;
  
  function enableAudioContext() {
      if (!audioContextEnabled) {
          // Create a silent audio to enable audio context
          var silentAudio = new Audio('data:audio/wav;base64,UklGRnoGAABXQVZFZm10IBAAAAABAAEAQB8AAEAfAAABAAgAZGF0YQoGAACBhYqFbF1fdJivrJBhNjVgodDbq2EcBj+a2/LDciUFLIHO8tiJNwgZaLvt559NEAxQp+PwtmMcBjiR1/LMeSwFJHfH8N2QQAoUXrTp66hVFApGn+DyvmYfCEORzPPMfS4HH2+57+ORPQ==');
          silentAudio.play().then(function() {
              audioContextEnabled = true;
          }).catch(function() {
              // Audio context still not enabled
          });
      }
  }
  ...
  function paysound() {
      ...
      if (playPromise !== undefined) {
          playPromise.then(function() {
              ...
              console.log("Payment sound autoplay prevented:", error);
              // Enable sound on next user interaction
              document.addEventListener('click', function enablePayAudioOnClick() {
                  audio.play().catch(function(e) {
                      console.log("Payment audio play failed:", e);
                  });
                  document.removeEventListener('click', enablePayAudioOnClick);
              }, { once: true });
          });
      }
  }
                      ...
                      if (myAudio) {
                          ...
                          if (playPromise !== undefined) {
                              playPromise.then(function() {
                                  ...
                                  // Autoplay was prevented, enable sound on next user interaction
                                  console.log("Audio autoplay prevented. Sound will play after user interaction.");
                      ...
                      if (myAudio) {
                          ...
                          if (playPromise !== undefined) {
                              playPromise.then(function() {
                                  ...
                                  // Autoplay was prevented, enable sound on next user interaction
                                  console.log("QR Audio autoplay prevented. Sound will play after user interaction.");
                                  document.addEventListener('click', function enableQRAudioOnClick() {
                                      myAudio.play().catch(function(e) {
                                          console.log("QR Audio play failed:", e);
                                      });
                                      document.removeEventListener('click', enableQRAudioOnClick);
                                  }, { once: true });
                              });
                          }
                      }
  ...
  function showhidecard(element) {
      ...
      var mpesaArea = $(element).closest('div.row').next().find('div.mpesa-area');
      
      console.log('Payment type changed to:', cardtype);

      // Hide all areas first
      $(element).closest('div.row').next().find('div.cardarea').addClass("display-none");
      $(element).closest('div.row').next().find('div.mpesa-area').addClass("display-none");
      ...
      if (cardtype == 4) {
          ...
          console.log('M-Pesa payment selected - showing MSDN input');
          // M-Pesa payment method
          $("#isonline").val(1);
          $(element).closest('div.row').next().find('div.mpesa-area').removeClass("display-none");
          $("#assigncard_terminal").val('');
          $("#assignbank").val('');
          $("#assignlastdigit").val('');
          
          // Focus on M-Pesa input after a short delay
          setTimeout(function() {
              $('#mpesa_number').focus();
          }, 100);
      } else {
          console.log('Other payment method selected:', cardtype);
          $("#isonline").val(1);
          $("#assigncard_terminal").val('');
          $("#assignbank").val('');
          $("#assignlastdigit").val('');
  ...
  function submitmultiplepay() {
      ...
      // Validate M-Pesa MSDN number if M-Pesa is selected
      var isMpesaSelected = false;
      $('select[name="paytype[]"]').each(function() {
          if ($(this).val() == 14) { // M-Pesa payment method ID
              isMpesaSelected = true;
              var mpesaNumber = $('#mpesa_number').val();
              ...
              if (!mpesaNumber || mpesaNumber.trim() === '') {
                  setTimeout(function() {
                      ...
                      toastr.error("Please enter M-Pesa mobile number (MSDN)", 'Error');
                  }, 100);
                  return false;
              }
          }
      ...
      // Check if M-Pesa is selected to show appropriate loading message
      var isMpesaPayment = false;
      $('select[name="paytype[]"]').each(function() {
          if ($(this).val() == 14) {
              isMpesaPayment = true;
          }
      ...
      $.ajax({
          ...
          success: function(data) {
              ...
              try {
                  // Try to parse JSON response for M-Pesa handling
                  var response = JSON.parse(data);
                  
                  if (response.status === 'pending_mpesa') {
                      // M-Pesa payment initiated - show success message with instructions
                      showMpesaInstructionsModal(response.transaction_ref, response.message);
                      return;
                      ...
                      // Payment successful (including completed M-Pesa)
                      ...
                      if (value == 1) {
                          setTimeout(function() {
                              ...
                              toastr.success(response.message || "Payment taken successfully", 'Success');
                  ...
                  // Not JSON, treat as regular response (successful payment)
              }
          ...
          error: function() {
              // Hide loading modals on error
              hideAllLoadingModals();
              toastr.error('Payment processing failed. Please try again.', 'Error');
          }

      });
  }

  // Loading and M-Pesa modal functions
  function showMpesaProcessingModal() {
      var modalHtml = `
          <div class="modal fade" id="mpesa-processing-modal" tabindex="-1" role="dialog" data-backdrop="static" data-keyboard="false">
      ...
      $('#mpesa-processing-modal').modal('show');
  }

  function showPaymentProcessingModal() {
      var modalHtml = `
          <div class="modal fade" id="payment-processing-modal" tabindex="-1" role="dialog" data-backdrop="static" data-keyboard="false">
              <div class="modal-dialog modal-sm" role="document">
                  <div class="modal-content">
                      <div class="modal-body text-center">
                          <div class="spinner-border text-primary mb-3" role="status">
  ...
  function showMpesaInstructionsModal(transactionRef, message) {
      // Hide processing modal first
      $('#mpesa-processing-modal').modal('hide');
      
      var modalHtml = `
          <div class="modal fade" id="mpesa-instructions-modal" tabindex="-1" role="dialog">
              <div class="modal-dialog" role="document">
                  <div class="modal-content">
                      <div class="modal-header bg-success text-white">
                          <h4 class="modal-title">
                              <i class="fa fa-mobile"></i> M-Pesa Payment Initiated
                          </h4>
                      </div>
                      <div class="modal-body text-center">
                          ...
                          <p><strong>Transaction Reference:</strong><br>
                             <code>${transactionRef}</code></p>
                          
                          <div class="alert alert-info">
                              <small><strong>Note:</strong> Please complete the payment on your phone. 
                              The order will be confirmed once payment is received.</small>
                          </div>
                      </div>
                      <div class="modal-footer">
                          <button type="button" class="btn btn-primary" onclick="closeMpesaInstructions()">
      ...
      $('body').append(modalHtml);
      $('#mpesa-instructions-modal').modal('show');
  }

  function closeMpesaInstructions() {
      $('#mpesa-instructions-modal').modal('hide');
      $('#mpesa-instructions-modal').remove();
      
      // Close payment modal and refresh the order list
      $('#payprint_marge').modal('hide');
      $('#modal-ajaxview').empty();
      prevsltab.trigger("click");
  }
  ...
  function hideAllLoadingModals() {
      ...
      // Remove modals from DOM after hiding
      setTimeout(function() {
          $('#mpesa-processing-modal').remove();
          $('#payment-processing-modal').remove();
      }, 500);
  }
  ...
  function checktable(id = null) {
      ...
      // No person entry required anymore
      var order_person = 0;
      ...
      if (order_person != "") {
          ...
          // No person selection needed; just set table and close modal
          if (id != null) {
              $('#tableid').val(id).trigger('change');
              $('#table_member_multi').val(0);
              $('#table_member_multi_person').val(0);
  ...
  function multi_table() {
      ...
      // No person input; default to 0
      $('#table_member').val(0);
      $('#table_person').val(0);
      $('#table_member_multi_person').val('');
  ...
  function opencashregister() {
      ...
      $.ajax({
          type: "POST",
          url: basicinfo.baseurl.replace(/\/+$/, '') + "/ordermanage/order/addcashregister",
          data: formdata,
          processData: false,
          contentType: false,
          success: function(data) {
  ...
  function closeopenresister() {
      var closeurl = basicinfo.baseurl.replace(/\/+$/, '') + "/ordermanage/order/cashregisterclose";
      var csrf = $('#csrfhashresarvation').val();
      $.ajax({
          type: "GET",
          async: false,
  ...
  function closecashregister() {
      ...
      $.ajax({
          type: "POST",
          url: basicinfo.baseurl.replace(/\/+$/, '') + "/ordermanage/order/closecashregister",
          data: formdata,
          processData: false,
          contentType: false,
          success: function(data) {
  ...
  // M-Pesa number validation and formatting
  $(document).on('input', '#mpesa_number', function() {
      // Basic validation - no visual styling
      var mpesaNumber = $(this).val();
      // Validation logic can be added here if needed
  });

  // Auto-format M-Pesa number
  $(document).on('blur', '#mpesa_number', function() {
      var value = $(this).val().replace(/\D/g, ''); // Remove non-digits
      
      // Auto-add 254 prefix if user starts with 07 or 01
