...
class Home_model extends CI_Model {
	...
	public function monthly_sales_total($start_date,$end_date)
	{
		$this->db->select('SUM(bill_amount) as total');
        $this->db->from('bill');
		$this->db->where('bill_status', 1);
		$this->db->where('create_at >=', $start_date.' 00:00:00');
		$this->db->where('create_at <=', $end_date.' 23:59:59');
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
	...
	public function monthly_discount_total($start_date,$end_date)
	{
		$this->db->select('SUM(discount) as total');
        $this->db->from('bill');
		$this->db->where('bill_status', 1);
		$this->db->where('create_at >=', $start_date.' 00:00:00');
		$this->db->where('create_at <=', $end_date.' 23:59:59');
	...
	public function today_discount_total()
	{
		$today=date('Y-m-d');
		$this->db->select('SUM(discount) as total');
        $this->db->from('bill');
		$this->db->where('bill_status', 1);
		$this->db->where('create_at >=', $today.' 00:00:00');
		$this->db->where('create_at <=', $today.' 23:59:59');
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
            return (float)$query->row()->total;  
        }
	...
	public function lifetime_orders_total()
	{
		$today = date('Y-m-d');
		$this->db->select('COUNT(order_id) as total');
        $this->db->from('customer_order');
		$this->db->where('order_status', 1); // Pending orders
		$this->db->where('order_date', $today); // Only today's orders
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
	...
	public function today_customers_total()
	{
		$today = date('Y-m-d');
		$this->db->select('COUNT(DISTINCT customer_id) as total');
        $this->db->from('customer_order');
		$this->db->where('order_date', $today);
		$this->db->where('order_status !=', 5); // Exclude cancelled orders
		$this->db->where('customer_id IS NOT NULL'); // Ensure customer_id exists
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
            return (int)$query->row()->total;  
        }
	...
	public function lifetime_dishes_total()
	{
		$this->db->select('SUM(menuqty) as total');
        ...
        $query = $this->db->get();
        if ($query->num_rows() > 0 && $query->row()->total !== null) {
            return (int)$query->row()->total;  
        }
        return 0;
	}

	public function delivered_dishes_total()
	{
		$this->db->select('SUM(menuqty) as total');
        $this->db->from('order_menu');
		$this->db->join('customer_order', 'order_menu.order_id = customer_order.order_id', 'left');
		$this->db->where('customer_order.order_status', 4); // Only completed/delivered orders
        $query = $this->db->get();
        if ($query->num_rows() > 0 && $query->row()->total !== null) {
            return (int)$query->row()->total;  
        }
	...
	public function pending_dishes_total()
	{
		$this->db->select('SUM(menuqty) as total');
        $this->db->from('order_menu');
		$this->db->join('customer_order', 'order_menu.order_id = customer_order.order_id', 'left');
		$this->db->where('customer_order.order_status !=', 4); // Exclude completed orders
		$this->db->where('customer_order.order_status !=', 5); // Exclude cancelled orders
        $query = $this->db->get();
        if ($query->num_rows() > 0 && $query->row()->total !== null) {
            return (int)$query->row()->total;  
