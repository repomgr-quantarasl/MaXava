...
class Hungry extends CI_Controller
{
	...
	public function checkavailablity()
	{
		...
		$newdate = $this->input->post('getdate');
			...
			if (empty($numofpeople) || empty($newdate) || empty($time)) {
				echo json_encode(array('error' => 'Missing required parameters'));
				return;
			}
			
			// Check if required tables exist
			if (!$this->db->table_exists('tblreservation')) {
				echo json_encode(array('error' => 'Reservation table not found'));
				return;
			}
			
			if (!$this->db->table_exists('rest_table')) {
				echo json_encode(array('error' => 'Restaurant table not found'));
				return;
			}
		...
		$data['tableinfo'] = $this->hungry_model->checkfree($gettable, $numofpeople);
				...
				// If no reserved tables, pass empty string to get all available tables
				$data['tableinfo'] = $this->hungry_model->checkfree('', $numofpeople);
			}
			
			// Get reservation settings with error handling
		$rseting = $this->hungry_model->read('*', 'setting', array('id' => 2));
			if (empty($rseting)) {
				echo json_encode(array('error' => 'Reservation settings not found'));
				return;
			}
			
			$openingtimerv = !empty($rseting->reservation_open) ? strtotime($rseting->reservation_open) : 0;
			$closetimerv = !empty($rseting->reservation_close) ? strtotime($rseting->reservation_close) : 0;
			$maxperson = !empty($rseting->maxreserveperson) ? (int)$rseting->maxreserveperson : 0;
		...
		$offinformation = $this->hungry_model->read('*', 'reservationofday', array('offdaydate' => $newdate));
			...
			// Check if off day information exists
			if (!empty($offinformation) && !empty($offinformation->offdaydate)) {
		$offdate = $offinformation->offdaydate;
				if (!empty($offinformation->availtime) && strpos($offinformation->availtime, '-') !== false) {
		...
		$curtime = strtotime(date("h:i:s A"));
			$totalperson = !empty($gettotalreserverp->totalperson) ? (int)$gettotalreserverp->totalperson : 0;
			$taken = $numofpeople + $totalperson;
		...
		$data['contactno'] = $this->input->post('contactno');
			...
			if ($maxperson > 0 && $maxperson < $numofpeople) {
			echo 1;
			exit;
		} else if ($maxperson < $taken) {
			echo 1;
			exit;
			} else if ($offdate && ($curtime >= $deltime1) && ($curtime < $deltime2) && (strtotime($offdate) == strtotime($newdate))) {
			echo 2;
		} else {
				// Check if view file exists before loading
				$view_path = 'themes/' . $this->themeinfo->themename . '/checkavail';
				if (file_exists(APPPATH . 'views/' . $view_path . '.php')) {
					$this->load->view($view_path, $data);
				} else {
					// Fallback if view doesn't exist
					echo json_encode(array('error' => 'View template not found'));
				}
			}
		} catch (Exception $e) {
			// Log the error and return a user-friendly message
			error_log('Reservation availability check error: ' . $e->getMessage());
			echo json_encode(array('error' => 'Unable to check availability. Please try again.'));
		}
	}
	...
	public function checkout()
	{
		...
		if (!empty($this->cart->contents())) {
			...
			$data['taxinfos'] = $this->taxchecking();
			$data['openclosetime'] =  $this->hungry_model->read_allorderby('*', 'tbl_openclose', 'stid', 'ASC');
			$data['tables'] = $this->reservation_model->table_dropdown();
			
			// Get selected shipping method details
			$selectedShippingId = $this->session->userdata('shippingid');
			if (!empty($selectedShippingId)) {
				$data['selected_shipping_method'] = $this->hungry_model->read('*', 'shipping_method', array('ship_id' => $selectedShippingId));
			} else {
				$data['selected_shipping_method'] = null;
			}
	...
	public function submitregister()
{
    ...
    $this->form_validation->set_rules(
        'user_email', 
        'Email', 
        'required|valid_email|is_unique[customer_info.customer_email]',
        array('is_unique' => 'Sorry, this Email address has already been used!')
    );
    $this->form_validation->set_rules('country_code', 'Country Code', 'required');
    $this->form_validation->set_rules(
        'phone',
        'Mobile',
        'required|regex_match[/^[0-9]{7,12}$/]|is_unique[customer_info.customer_phone]',
        array(
            'regex_match' => 'Please enter a valid phone number (digits only).',
            'is_unique'   => 'Sorry, this Mobile number has already been used!'
        )
    );
    $this->form_validation->set_rules('u_pass', 'Password', 'required|min_length[6]');
    ...
    if ($this->form_validation->run() == FALSE) {
        ...
        $sl = $lastid->cuntomer_no ?? "cus-0000";
        $supno = explode('-', $sl);
        $nextno = $supno[1] + 1;
        $cutstr = substr('0000', strlen((int)$nextno));
        $sino = $supno[0] . "-" . $cutstr . $nextno;
        ...
        $pointsys = "";
        foreach ($scan as $file) {
            if ($file == "loyalty" && file_exists(APPPATH . 'modules/' . $file . '/assets/data/env')) {
                $pointsys = 1;
            }
        }
        ...
        $indata['customer_phone']   = $this->input->post('phone', TRUE);        // digits only
        $indata['country_code']     = $this->input->post('country_code', TRUE);  // country code
	...
	public function _sendingForgotPassMail($data)
	{
		...
		$message    = "Suite  votre demande, nous vous avons envoyé vos identifiants de connexion -
		<br><br>
	...
	public function placeorder()
	{
		...
		if (empty($islogin)) {
			if (!empty($emailexists)) {
    // Auto-login existing customer instead of error
    $mysesdata = array('CusUserID' => $emailexists->customer_id);
    $this->session->set_userdata($mysesdata);
    ...
    $this->session->set_userdata($mysesdata);
		...
		// Check if table selection is needed for dine-in
		$tableid = $this->input->post('tableid');
		$shippingmethod = $this->session->userdata('shippingid');
		$shippinginfo = $this->hungry_model->read('*', 'shipping_method', array('ship_id' => $shippingmethod));
		
		if(!empty($shippinginfo) && ($shippinginfo->shipping_method == 'Dine-in' || $shippinginfo->shipping_method == 'Consumir no Local') && !empty($tableid)) {
			$orderinfo['table_no'] = $tableid;
		} else {
...
document.getElementById("paytrack").click();
  function payWithPaystack(){
    ...
    handler.openIframe();
			...
			} else if ($paymentsatus == 14) {
				redirect('hungry/mpesa/' . $orderid . '/2');
  ...
  function payWithPaystack(){
	...
	public function successful2($page = null)
	{
		...
		if ($page == 1) {
			...
			$msg = [
				'title' => 'Commande passe avec succès !!',
				'body' => 'Votre identifiant de commande: ' . $orderid . ' Placé avec succès. Veuillez attendre servi',
				'icon' => 'img/icon.png',
				'image' => 'img/d.png',
			];
	...
	public function orderdeails($orderid)
	{
		$data['title'] = "Order Details & Invoice";
		$data['seoterm'] = "order_details";
		
		// Get order information
		$orderinfo = $this->hungry_model->read('*', 'customer_order', array('order_id' => $orderid));
		if (empty($orderinfo)) {
			$this->session->set_flashdata('exception', 'Order not found!');
			redirect('menu');
			return;
		}
		...
		$data['billinfo'] = $this->hungry_model->billinfo($orderid);
		$data['iteminfo'] = $this->hungry_model->customerorder($orderid);
		$data['customerinfo'] = $this->hungry_model->read('*', 'customer_info', array('customer_id' => $orderinfo->customer_id));
		$data['openclosetime'] = $this->hungry_model->read_allorderby('*', 'tbl_openclose', 'stid', 'ASC');
		
		// Get payment method info
		if (!empty($data['billinfo']->payment_method_id)) {
			$data['paymentmethod'] = $this->hungry_model->read('*', 'payment_method', array('payment_method_id' => $data['billinfo']->payment_method_id));
		}
		
		// Get M-Pesa transaction info if it's an M-Pesa payment
		if (!empty($data['billinfo']->payment_method_id) && $data['billinfo']->payment_method_id == 14) {
			$data['mpesa_transaction'] = $this->db->select('*')->from('mpesa_transactions')->where('order_id', $orderid)->get()->row();
		}
		
		// Store settings for tax calculation, etc.
		$data['storesetting'] = $this->db->select('*')->from('setting')->get()->row();
		
		// Currency info
		$data['currency'] = $this->storecurrency;
		
		// Back button destination - check if user is logged in
		$islogin = $this->session->userdata('CusUserID');
		if ($islogin) {
			$data['back_url'] = base_url('myorderlist');
			$data['back_text'] = 'My Orders';
		} else {
			$data['back_url'] = base_url('menu');
			$data['back_text'] = 'Back to Menu';
		}
		
		// Check if order belongs to current user (security check)
		if ($islogin && $orderinfo->customer_id != $islogin) {
			$this->session->set_flashdata('exception', 'Access denied! You can only view your own orders.');
			redirect('myorderlist');
			return;
		}
		
		// Load the view
		$data['content'] = $this->load->view('themes/' . $this->themeinfo->themename . '/orderdetails', $data, TRUE);
		$this->load->view('themes/' . $this->themeinfo->themename . '/index', $data);
	}

	// M-Pesa Payment Processing
	public function mpesa($orderid, $page = null)
	{
		$data['title'] = "M-Pesa Payment";
		$data['seoterm'] = "payment"; // Add SEO term for proper page metadata
		$data['orderinfo'] = $this->hungry_model->read('*', 'customer_order', array('order_id' => $orderid));
		$data['paymentinfo'] = $this->hungry_model->read('*', 'paymentsetup', array('paymentid' => 14));
		$data['customerinfo'] = $this->hungry_model->read('*', 'customer_info', array('customer_id' => $data['orderinfo']->customer_id));
		$bill = $this->hungry_model->read('*', 'bill', array('order_id' => $orderid));
		$data['billinfo'] = $this->hungry_model->read('*', 'bill_card_payment', array('bill_id' => $bill->bill_id));
		$data['page'] = $page;
		...
		$data['content'] = $this->load->view('themes/' . $this->themeinfo->themename . '/mpesa', $data, TRUE);
		$this->load->view('themes/' . $this->themeinfo->themename . '/index', $data);
	}

	// Process M-Pesa payment
	public function process_mpesa()
	{
		$this->load->library('mpesa_lib');
		
		// Get input data (support both POST form and JSON)
		...
		if (empty($input)) {
			...
			$orderid = $input['order_id'] ?? $input['reference'] ?? '';
			$phone = $input['phone'] ?? '';
			$amount = $input['amount'] ?? '';
		...
		log_message('debug', 'M-Pesa Process Data - Phone: ' . $phone . ', Amount: ' . $amount . ', OrderID: ' . $orderid);
		...
		// For testing, create a dummy order if no order_id provided
		if (empty($orderid)) {
			$orderid = 'TEST_' . time();
		...
		$paymentinfo = $this->hungry_model->read('*', 'paymentsetup', array('paymentid' => 14));
		if (empty($paymentinfo)) {
			echo json_encode([
				'success' => false,
				'message' => 'M-Pesa payment not configured'
			]);
			return;
		}
		
		// Configure M-Pesa library
		$config = array(
			'api_key' => $paymentinfo->marchantid,
			'public_key' => $paymentinfo->password,
			'service_provider_code' => $paymentinfo->email,
			'is_live' => $paymentinfo->Islive
		);
		
		$this->mpesa_lib->initialize($config);
		
		// Debug: Log phone number before validation
		log_message('debug', 'M-Pesa Phone Before Validation: ' . $phone);
		
		// Validate phone number
		$validPhone = $this->mpesa_lib->validatePhoneNumber($phone);
		
		// Debug: Log validation result
		log_message('debug', 'M-Pesa Phone After Validation: ' . ($validPhone ? $validPhone : 'FAILED'));
		
		if (!$validPhone) {
			echo json_encode([
				'success' => false,
				'message' => 'Invalid phone number format. Please use Mozambique format (e.g., 258841234567). Received: ' . $phone
			]);
			return;
		}
		
		// 1. Create initial multipay_bill record (following standard pattern)
		$multipay = array(
			'order_id'        => $orderid,
			'payment_type_id' => 14,  // M-Pesa payment method ID
			'amount'          => $amount
		);
		...
		$paymentData = array(
			...
			'phone' => $validPhone,
			'amount' => $amount,
			'user_reference' => 'ORDER_' . $orderid,
			'multipay_id' => $multipay_id  // Link to multipay record
		);
		
		$result = $this->mpesa_lib->processPayment($paymentData);
		
		// 3. SYNCHRONOUS PROCESSING - Handle M-Pesa response immediately
		if ($result['success']) {
			// Payment was successful - update all database tables synchronously
			try {
				...
				$this->db->where('order_id', $orderid);
				$this->db->update('bill', $updateData);
				
				// Update order status (mark as completed/served)
				$updateData2 = array('order_status' => 4);
				$this->db->where('order_id', $orderid);
				$this->db->update('customer_order', $updateData2);
				
				// Create accounting entry (following system pattern)
				$orderinfo = $this->db->select('*')->from('customer_order')->where('order_id', $orderid)->get()->row();
				if ($orderinfo) {
					// Get M-Pesa chart of accounts code
					$paytype = $this->db->select('payment_method')->from('payment_method')->where('payment_method_id', 14)->get()->row();
					$coainfo = $this->db->select('HeadCode')->from('acc_coa')->where('HeadName', $paytype->payment_method)->get()->row();
					$headcode = !empty($coainfo) ? $coainfo->HeadCode : '1020101'; // Default to cash if not found
					...
					$cusifo = array(
						...
						'VDate'          => date('Y-m-d'),
						'COAID'          => $headcode,
						'Narration'      => 'M-Pesa Payment for Order #' . $orderid . ' - Synchronous Processing',
						'Debit'          => $amount,
						'Credit'         => 0,
						'IsPosted'       => 1,
						'CreateBy'       => 'mpesa_sync_system',
						'CreateDate'     => date('Y-m-d H:i:s'),
						'IsAppove'       => 1
					);
					$this->db->insert('acc_transaction', $cusifo);
				}
				
				// Process loyalty points and customer benefits
				$this->process_mpesa_success($orderid, $result['transaction_id'] ?? null, $amount);
				
				// Log successful synchronous processing
				log_message('info', 'M-Pesa Synchronous Processing Complete for Order: ' . $orderid);
				
				// Update result message to indicate synchronous processing
				$result['message'] = 'Payment successful and order processed immediately';
				$result['sync_processed'] = true;
				$result['database_updated'] = [
					'bill_status' => 'Updated to paid (1)',
					'order_status' => 'Updated to completed (4)',
					'accounting_entry' => 'Created',
					'loyalty_points' => 'Processed'
				];
				
			} catch (Exception $e) {
				// If database update fails, log error but don't fail the payment
				log_message('error', 'M-Pesa Synchronous DB Update Failed: ' . $e->getMessage() . ' for Order: ' . $orderid);
				$result['sync_processed'] = false;
				$result['sync_error'] = $e->getMessage();
				$result['message'] .= ' (Note: Payment successful but order processing had issues)';
			}
			
		} else {
			// Payment failed - remove the multipay record
			$this->db->where('multipay_id', $multipay_id);
			$this->db->delete('multipay_bill');
			
			// Log failed payment
			log_message('error', 'M-Pesa Payment Failed for Order: ' . $orderid . ' - ' . $result['message']);
		}
	...
	// 		$transactionRef = $input['input_TransactionReference'] ?? '';
	// 		$status = $input['output_ResponseCode'] == 'INS-0' ? 'SUCCESS' : 'FAILED';
			
	// 		$additionalData = array(
	// 			'mpesa_response_code' => $input['output_ResponseCode'] ?? null,
	// 			'mpesa_response_desc' => $input['output_ResponseDesc'] ?? null,
	// 			'mpesa_conversation_id' => $input['output_ConversationID'] ?? null,
	// 			'mpesa_transaction_id' => $input['output_TransactionID'] ?? null,
	// 			'raw_response' => json_encode($input)
	// 		);
			
	// 		$this->mpesa_lib->updateTransactionStatus($transactionRef, $status, $additionalData);
			
	// 		// If successful, update order status
	// 		if ($status == 'SUCCESS') {
	// 			$transaction = $this->mpesa_lib->checkTransactionStatus($transactionRef);
	// 			if ($transaction) {
	// 				// 1. Add entry to multipay_bill table (following PayPal pattern)
	// 				$multipay = array(
	...
	// 				// 2. Update bill status (mark as paid)
	// 				$updateData = array(
	// 					'bill_status' => 1,
	// 					'create_at'   => date('Y-m-d H:i:s')
	// 				);
	...
	// 				// 3. Update order status (mark as completed/served)
	// 				$updateData2 = array('order_status' => 4);
	// 				$this->db->where('order_id', $transaction->order_id);
	// 				$this->db->update('customer_order', $updateData2);
					
	// 				// 4. Create accounting entry (following system pattern)
	// 				$orderinfo = $this->db->select('*')->from('customer_order')->where('order_id', $transaction->order_id)->get()->row();
	// 				if ($orderinfo) {
	// 					$cusinfo = $this->db->select('*')->from('customer_info')->where('customer_id', $orderinfo->customer_id)->get()->row();
	...
	// 					// Create accounting transaction
	// 					$cusifo = array(
	// 						'VNo'            => $transaction->order_id,
	// 						'Vtype'          => 'Sales',
	// 						'VDate'          => date('Y-m-d'),
	// 						'COAID'          => $headcode,
	// 						'Narration'      => 'M-Pesa Payment for Order #' . $transaction->order_id,
	// 						'Debit'          => $transaction->amount,
	// 						'Credit'         => 0,
	// 						'IsPosted'       => 1,
	// 						'CreateBy'       => 'mpesa_system',
	// 						'CreateDate'     => date('Y-m-d H:i:s'),
	// 						'IsAppove'       => 1
	// 					);
	// 					$this->db->insert('acc_transaction', $cusifo);
	...
	// Helper method to process successful M-Pesa payments (following PayPal pattern)
	private function process_mpesa_success($orderid, $transaction_id = null, $amount = null)
	{
		...
		// Handle loyalty points (following PayPal success pattern)
		$scan = scandir('application/modules/');
		$getcus = "";
		foreach ($scan as $file) {
			if ($file == "loyalty") {
	...
	// M-Pesa success page (following PayPal pattern)
	public function mpesa_successful($orderid, $page = null)
	{
		// // Process success logic (loyalty points, etc.)
		// $this->process_mpesa_success($orderid);
		...
		if ($page == 1) {
			...
			// Web success
			redirect('hungry/orderdeails/' . $orderid);
		}
	}

	// Test M-Pesa phone validation (for debugging)
	public function test_mpesa_phone()
	{
		$this->load->library('mpesa_lib');
		
		$input = json_decode(file_get_contents('php://input'), true);
		$phone = $input['phone'] ?? $this->input->post('phone') ?? '';
		...
		echo json_encode([
			'success' => $validPhone !== false,
			'original_phone' => $phone,
			'validated_phone' => $validPhone,
			'phone_length' => strlen(preg_replace('/[^0-9]/', '', $phone)),
			'cleaned_phone' => preg_replace('/[^0-9]/', '', $phone),
			'debug_info' => [
				'starts_with_258' => substr(preg_replace('/[^0-9]/', '', $phone), 0, 3) === '258',
				'operator_code' => substr(preg_replace('/[^0-9]/', '', $phone), 3, 2),
				'is_valid_operator' => in_array(substr(preg_replace('/[^0-9]/', '', $phone), 3, 2), ['82', '83', '84', '85', '86', '87'])
			]
		]);
	}

	// Test M-Pesa RSA encryption (for debugging)  
	public function test_mpesa_rsa()
	{
		$this->load->library('mpesa_lib');
		
		// Get M-Pesa config from database
		$paymentinfo = $this->hungry_model->read('*', 'paymentsetup', array('paymentid' => 14));
		
		if (empty($paymentinfo)) {
			echo json_encode([
				'success' => false,
				'message' => 'M-Pesa not configured in database'
			]);
			return;
		}
		...
		try {
			...
			$testPhone = '258842085812';
			$validPhone = $this->mpesa_lib->validatePhoneNumber($testPhone);
			
			echo json_encode([
				'success' => true,
				'message' => 'M-Pesa configuration test successful',
				'config_loaded' => [
					'api_key_length' => strlen($paymentinfo->marchantid),
					'public_key_length' => strlen($paymentinfo->password),
					'service_provider_code' => $paymentinfo->email,
					'is_live' => $paymentinfo->Islive
				],
				'phone_validation' => [
			...
			echo json_encode([
				'success' => false,
				'message' => 'RSA encryption test failed: ' . $e->getMessage(),
				'config_info' => [
					'api_key_present' => !empty($paymentinfo->marchantid),
					'public_key_present' => !empty($paymentinfo->password),
					'service_provider_present' => !empty($paymentinfo->email)
				]
			]);
		}
	}
