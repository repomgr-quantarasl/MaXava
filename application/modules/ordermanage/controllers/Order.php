...
class Order extends MX_Controller {
	...
	public function insert_customer(){
	  ...
	  $this->form_validation->set_rules('customer_name', 'Customer Name'  ,'|max_length[100]');
	...
	public function placeoreder(){
		...
		$this->form_validation->set_rules('tableid','Select Table','');
	...
	public function orderdetails($id){
		$this->permission->method('ordermanage','read')->redirect();
		   ...
		   $settinginfo=$this->order_model->settinginfo();
		   $data['storeinfo']      = $settinginfo;
		   $data['settinginfo']    = $settinginfo; // Add settinginfo to data array
	       $data['currency']=$this->order_model->currencysetting($settinginfo->currency);
		   $data['title']          = display('order_details'); // Add title variable
	...
	public function kitchen($kitchenid=null){
		...
		// Test method to debug the itemacepted issue
		public function test_itemacepted(){
			try {
		...
		public function itemacepted(){
			...
			try {
				...
				if(empty($orderid) || empty($kitid) || empty($itemid)) {
					echo 'error: Missing required fields';
					return;
				}
				
				// Log the received data for debugging
				log_message('debug', 'itemacepted called with orderid: '.$orderid.', kitid: '.$kitid.', itemid: '.$itemid.', varient: '.$varient);
				
				$itemids=explode(',',$itemid);
				$varientids=explode(',',$varient);
				// Note: The original lines with array_values(trim()) were incorrect and removed
				$i=0;
				foreach($itemids as $sitem){
					$vaids=$varientids[$i];
				...
				log_message('error', 'Error in itemacepted: ' . $e->getMessage());
	...
	public function markasdone(){
		...
		try {
			...
			if (empty($varient)) {
				throw new Exception('Variant is required');
			}
			
			$itemids = explode(',', $itemid);
			$varientids = explode(',', $varient);
			
			// Validate that item and variant arrays have the same count
			if (count($itemids) !== count($varientids)) {
				throw new Exception('Item and variant arrays must have the same number of elements');
			}
			
			$i = 0;
			foreach($itemids as $sitem){
				// Ensure we have a valid variant for this item
				if (!isset($varientids[$i])) {
					throw new Exception('Missing variant for item at index ' . $i);
				}
				$vaids = $varientids[$i];
				$updatetready = array(
						'food_status'           => 1,
		   ...
		   $this->load->view('kitchen_view',$data);
			...
			log_message('error', 'Error in markasdone method: ' . $e->getMessage());
			...
			$this->output
				...
				->set_output(json_encode(array(
					'status' => 'error',
					'message' => 'Failed to mark order as done: ' . $e->getMessage()
				)));
			return;
		}
	}
	...
	public function paymultiple(){
		...
		$i=0;
		$billinfo = $this->db->select('*')->from('bill')->where('order_id',$orderid)->get()->row();
		$mpesa_numbers = $this->input->post('mpesa_number', true);
		
		foreach ($payamonts  as $payamont) {
			$paidamount = $paidamount+$payamont;
			
			// Check if this is M-Pesa payment
			if ($paytype[$i] == 14) {
				// Process M-Pesa payment
				$mpesa_number = isset($mpesa_numbers[$i]) ? $mpesa_numbers[$i] : '';
				
				$data_pay = array('paytype' => $paytype[$i],'cterminal' => '',
					'mybank' => '','mydigit' => $mpesa_number,'payamont' => $payamont);
			} else {
				$data_pay = array('paytype' => $paytype[$i],'cterminal' => $cterminal[$i],
					'mybank' => $mybank[$i],'mydigit' => $mydigit[$i],'payamont' => $payamont);
			}
			
			$multipay_id = $this->add_multipay($orderid,$billinfo->bill_id,$data_pay);
			
			// Process M-Pesa payment after record is created
			if ($paytype[$i] == 14 && !empty($mpesa_number)) {
				$mpesa_payment_data = array(
					'amount' => $payamont,
					'msdn_number' => $mpesa_number,
					'order_id' => $orderid,
					'multipay_id' => $multipay_id
				);
				
				$mpesa_result = $this->process_mpesa_payment($mpesa_payment_data);
				
				if ($mpesa_result['status'] == 'error') {
					// M-Pesa payment failed, return error response
					echo json_encode(array(
						'success' => false,
						'message' => 'M-Pesa payment failed: ' . $mpesa_result['message']
					));
					return;
				} elseif ($mpesa_result['status'] == 'pending_mpesa') {
					// M-Pesa payment initiated successfully - show instructions to user
					echo json_encode(array(
						'success' => true,
						'status' => 'pending_mpesa',
						'message' => $mpesa_result['message'],
						'transaction_ref' => $mpesa_result['transaction_ref'],
						'order_id' => $orderid
					));
					return;
				}
...
//  		if(empty($checkuser)){
//  			if(!empty($openamount) && $openamount->closing_balance > '0.000'){
...
//  	 }
 ...
 public function closecashregister(){
	 ...
	 * Process M-Pesa payment
	 */
	public function process_mpesa_payment($payment_data) {
		// Load M-Pesa library
		$this->load->library('Mpesa_lib');
		
		try {
			// Get payment setup info for M-Pesa (payment method id = 14)
			$paymentinfo = $this->order_model->read('*', 'paymentsetup', array('paymentid' => 14));
			...
			$validPhone = $this->mpesa_lib->validatePhoneNumber($payment_data['msdn_number']);
			if (!$validPhone) {
				// If validation fails, try to use the phone number as-is for testing
				// Log the validation failure but proceed
				error_log('M-Pesa Phone validation failed for: ' . $payment_data['msdn_number']);
				$validPhone = $payment_data['msdn_number']; // Use original number for now
			}
			
			// Get customer information from order
			$orderinfo = $this->order_model->read('*', 'customer_order', array('order_id' => $payment_data['order_id']));
			$customer_id = $orderinfo ? $orderinfo->customer_id : 0;
			
			// Validate minimum amount (M-Pesa typically requires minimum 1 unit)
			if ((float)$payment_data['amount'] < 1) {
				return array(
					'status' => 'error',
					'message' => 'Amount too small. Minimum amount is 1.00'
				);
			}
			
			// Format amount - ensure it's a clean numeric value
			$formatted_amount = (float)$payment_data['amount'];
			
			// Log for debugging
			error_log("M-Pesa Amount Debug - Original: " . $payment_data['amount'] . ", Formatted: " . $formatted_amount);
			
			// Prepare M-Pesa payment data for the library
			$mpesa_data = array(
				'phone' => $validPhone,
				'amount' => $formatted_amount,
				'order_id' => $payment_data['order_id'],
				'customer_id' => $customer_id,
				'user_reference' => 'ORDER_' . $payment_data['order_id']
			);
			
			// Debug: Log the data being sent to M-Pesa
			error_log("M-Pesa Payment Data: " . json_encode($mpesa_data));
			
			// Process M-Pesa payment
			$response = $this->mpesa_lib->processPayment($mpesa_data);
			
			// Debug: Log the response from M-Pesa
			error_log("M-Pesa Response: " . json_encode($response));
			
			if ($response && isset($response['success']) && $response['success'] == true) {
				// Payment initiated successfully - customer will receive USSD prompt
				$transaction_reference = $response['transaction_id'] ?? '';
				
				// Store M-Pesa transaction reference
				$update_data = array(
					'multipayid' => $transaction_reference
				);
				
				$this->db->where('multipay_id', $payment_data['multipay_id']);
				$this->db->update('multipay_bill', $update_data);
				
				return array(
					'status' => 'pending_mpesa',
					'message' => 'M-Pesa payment initiated successfully! Please check your phone and complete the payment using USSD *150*00#',
					'transaction_ref' => $transaction_reference
				);
			} else {
				// Payment initiation failed
				$error_message = isset($response['message']) ? $response['message'] : 'M-Pesa payment failed to initiate';
	...
	public function check_mpesa_status() {
		...
		if (empty($transaction_ref) || empty($order_id)) {
			echo json_encode(array(
				'success' => false,
				'message' => 'Missing transaction reference or order ID'
			));
			return;
		}
		...
		try {
			// Get payment setup info for M-Pesa
			$paymentinfo = $this->order_model->read('*', 'paymentsetup', array('paymentid' => 14));
			if (empty($paymentinfo)) {
				echo json_encode(array(
					'success' => false,
			...
			$status_response = $this->mpesa_lib->checkTransactionStatus($transaction_ref);
			
			if ($status_response && isset($status_response['status'])) {
				if ($status_response['status'] == 'SUCCESS') {
					// Payment completed successfully - complete the order
					$this->complete_mpesa_order($order_id, $transaction_ref);
				...
				} elseif ($status_response['status'] == 'PENDING') {
					echo json_encode(array(
						'success' => true,
						'status' => 'pending',
						'message' => 'M-Pesa payment is still pending. Please complete on your phone.'
					));
				} else {
					echo json_encode(array(
						'success' => false,
						'status' => 'failed',
						'message' => 'M-Pesa payment failed or was cancelled'
					));
				}
			} else {
				echo json_encode(array(
					'success' => false,
					'message' => 'Unable to check M-Pesa payment status'
				));
			}
			...
			echo json_encode(array(
				'success' => false,
				'message' => 'Error checking M-Pesa status: ' . $e->getMessage()
			));
		}
	}
	
	/**
	 * Complete M-Pesa order after payment confirmation
	 */
	private function complete_mpesa_order($order_id, $transaction_ref) {
		// Update order status to paid
		$orderinfo = $this->order_model->uniqe_order_id($order_id);
		
		$updateData = array(
			'order_status' => 4, // Paid status
		...
		// Update bill status
		$this->db->where('order_id', $order_id);
		$this->db->update('bill', array('bill_status' => 1));
		
		// Update multipay_bill with final transaction reference
		$this->db->where('order_id', $order_id);
		$this->db->where('payment_type_id', 14);
		$this->db->update('multipay_bill', array('multipayid' => $transaction_ref));
		
		// Log the completion
		$logData = array(
			'action_page' => 'M-Pesa Payment',
			'action_done' => 'Payment Completed',
			'remarks' => 'M-Pesa payment completed for Order #' . $order_id,
			'user_name' => $this->session->userdata('fullname'),
			'entry_date' => date('Y-m-d H:i:s'),
		);
	...
	public function mpesa_callback() {
		...
		if ($callback_data && isset($callback_data['Body'])) {
			$result = $callback_data['Body'];
			$transaction_ref = $result['stkCallback']['CheckoutRequestID'] ?? '';
			$result_code = $result['stkCallback']['ResultCode'] ?? '';
			
			if ($result_code == '0') {
				// Payment successful
				$order_id = $this->extract_order_id_from_callback($callback_data);
				if ($order_id) {
					$this->complete_mpesa_order($order_id, $transaction_ref);
				}
			}
	...
	private function extract_order_id_from_callback($callback_data) {
		// Extract order ID from callback data - implementation depends on how order ID is stored
		// This might be in the transaction reference or metadata
		return null; // Implement based on your callback structure
	}
}
