...
public function create_s_setup(){ 
    ...
    // $this->form_validation->set_rules('amount[]', display('amount')); // removed: not required
    ...
    if ($this->form_validation->run() === true) {
        ...
        if (!empty($amount)) {
            ...
            // If no addition/deduction given, still save default record
            $postData = [
                'employee_id'    => $this->input->post('employee_id',true),
                'sal_type'       => $this->input->post('sal_type',true),
                'salary_type_id' => 0,
...
{ 
    ...
    if ($this->Payroll_model->emp_salstup_delete($id)) {   // FIXED
        $this->session->set_flashdata('message', display('delete_successfully'));
    } else {
        $this->session->set_flashdata('exception', display('please_try_again'));
    }
...
public function delete_sal_setup($id = null) 
	{ 
		$this->permission->module('hrm','delete')->redirect();
